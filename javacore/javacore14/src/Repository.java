...
public class Repository {
    ...
    public void filterProductByFood() {
        // for (Product product : listProducts) {
        // if (product.getCategory().equals(Category.FOOD)) {
        // System.out.println(product);
        // }
        // }
        listProducts.stream().filter(product -> product.getCategory().equals(Category.FOOD))
                .forEach(System.out::println);
    }

    public void filterProductByHouseWare() {
        listProducts.stream().filter(product -> product.getCategory().equals(Category.HOUSEWARE))
                .forEach(System.out::println);
    }

    public void filterProductByCosmetics() {
        listProducts.stream().filter(product -> product.getCategory().equals(Category.COSMETICS))
                .forEach(System.out::println);
    }

    public void filterProductByFashion() {
        listProducts.stream().filter(product -> product.getCategory().equals(Category.FASHION))
                .forEach(System.out::println);
    }
